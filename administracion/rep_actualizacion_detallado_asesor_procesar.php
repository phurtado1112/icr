...
// Pie de página
        ...
        while ($fila00 = bd_obtener_fila($res00)) {
            ...
            $consulta2 = "SELECT agente, idusuario FROM incaecrm.transacciones_view where idprograma="
                    . $idprograma . " and idtipificacion=14 and idcampania=" . $fila00['idcampania'];
            $res2 = bd_ejecutar_sql($consulta2);

            while ($fila2 = bd_obtener_fila($res2)) {
                $pdf->Cell(0, 8, utf8_decode($fila2['agente']), 0, 1, 'C');

                $pdf->Ln(5);

                $consulta = "SELECT count(idcliente) FROM incaecrm.transacciones_view where idprograma="
                        . $idprograma . " and idtipificacion=14 and idcampania=" . $fila00['idcampania']
                        . " and idusuario=" . $fila2['idusuario'];
                $res = bd_ejecutar_sql($consulta);
                $pdf->SetFont('Arial', '', 12);
                while ($fila = bd_obtener_fila($res)) {
                    $pdf->Cell(75, 6, utf8_decode('Total de Actualizados por Asesor: '), 1, 0, 'L', true);
                ...
                $pdf->Cell(0, 8, utf8_decode('1.- Detalle de Contactos Actualizados'), 0, 1, 'L');
                ...
                $consulta30 = "select nombre, email, pais
                    ...
                    where idprograma=" . $idprograma . " and idcampania='" . $fila00['idcampania']
                        . "' and tra.idtipificacion=14 and idusuario=" . $fila2['idusuario'] . "  and (fecha >='"
