...
<?php include_once "estado_de_campaf1assmryinfo.php" ?>
<?php
...
$Estado_de_CampaF1as_summary = NULL; // Initialize page object first

class crEstado_de_CampaF1as_summary extends crEstado_de_CampaF1as {
	...
	// Page object name
	var $PageObjName = 'Estado_de_CampaF1as_summary';
	...
	function __construct() {
		...
		// Table object (Estado_de_CampaF1as)
		if (!isset($GLOBALS["Estado_de_CampaF1as"])) {
			$GLOBALS["Estado_de_CampaF1as"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["Estado_de_CampaF1as"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Estado de Campañas', TRUE);
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmailText")) . "\" id=\"emf_Estado_de_CampaF1as\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_Estado_de_CampaF1as',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'Estado_de_CampaF1as_programa') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'Estado_de_CampaF1as_campania') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'Estado_de_CampaF1as_asesor') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'Estado_de_CampaF1as_fechatrans') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_Estado_de_CampaF1as_programa"])) {
			$this->LoadSelectionFromSession('programa');
		} elseif (@$_SESSION["sel_Estado_de_CampaF1as_programa"] == EWR_INIT_VALUE) { // Select all
			$this->programa->SelectionList = "";
		}

		// Get campania selected values
		if (is_array(@$_SESSION["sel_Estado_de_CampaF1as_campania"])) {
			$this->LoadSelectionFromSession('campania');
		} elseif (@$_SESSION["sel_Estado_de_CampaF1as_campania"] == EWR_INIT_VALUE) { // Select all
			$this->campania->SelectionList = "";
		}

		// Get asesor selected values
		if (is_array(@$_SESSION["sel_Estado_de_CampaF1as_asesor"])) {
			$this->LoadSelectionFromSession('asesor');
		} elseif (@$_SESSION["sel_Estado_de_CampaF1as_asesor"] == EWR_INIT_VALUE) { // Select all
			$this->asesor->SelectionList = "";
		}

		// Get fechatrans selected values
		if (is_array(@$_SESSION["sel_Estado_de_CampaF1as_fechatrans"])) {
			$this->LoadSelectionFromSession('fechatrans');
		} elseif (@$_SESSION["sel_Estado_de_CampaF1as_fechatrans"] == EWR_INIT_VALUE) { // Select all
			$this->fechatrans->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			if ($this->Export == "") {
				...
				$this->CALIFICADO->LinkAttrs["onclick"] = ewr_DrillDownJs($drillurl, 'Estado_de_CampaF1as_CALIFICADO', $GLOBALS["ReportLanguage"]->TablePhrase('Detalle_de_contacto_atendido', 'TblCaption'), $this->UseDrillDownPanel);
			}
			...
			if ($this->Export == "") {
				...
				$this->NOINTERESADO->LinkAttrs["onclick"] = ewr_DrillDownJs($drillurl, 'Estado_de_CampaF1as_NOINTERESADO', $GLOBALS["ReportLanguage"]->TablePhrase('Detalle_de_contacto_atendido', 'TblCaption'), $this->UseDrillDownPanel);
			}
			...
			if ($this->Export == "") {
				...
				$this->OTROPROGRAMA->LinkAttrs["onclick"] = ewr_DrillDownJs($drillurl, 'Estado_de_CampaF1as_OTROPROGRAMA', $GLOBALS["ReportLanguage"]->TablePhrase('Detalle_de_contacto_atendido', 'TblCaption'), $this->UseDrillDownPanel);
			}
			...
			if ($this->Export == "") {
				...
				$this->FALLIDA->LinkAttrs["onclick"] = ewr_DrillDownJs($drillurl, 'Estado_de_CampaF1as_FALLIDA', $GLOBALS["ReportLanguage"]->TablePhrase('Detalle_de_contacto_atendido', 'TblCaption'), $this->UseDrillDownPanel);
			}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_Estado_de_CampaF1as_$parm"] = "";
		$_SESSION["rf_Estado_de_CampaF1as_$parm"] = "";
		$_SESSION["rt_Estado_de_CampaF1as_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_Estado_de_CampaF1as_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_Estado_de_CampaF1as_$parm"];
		$fld->RangeTo = @$_SESSION["rt_Estado_de_CampaF1as_$parm"];
	}
...
// Create page object
if (!isset($Estado_de_CampaF1as_summary)) $Estado_de_CampaF1as_summary = new crEstado_de_CampaF1as_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$Estado_de_CampaF1as_summary;
...
// Create page object
var Estado_de_CampaF1as_summary = new ewr_Page("Estado_de_CampaF1as_summary");

// Page properties
Estado_de_CampaF1as_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = Estado_de_CampaF1as_summary.PageID;

// Extend page with Chart_Rendering function
Estado_de_CampaF1as_summary.Chart_Rendering = 
...
Estado_de_CampaF1as_summary.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
<?php include "estado_de_campaf1assmrypager.php" ?>
</div>
<?php } ?>
</td></tr></table>
<span data-class="tpb<?php echo $Page->GrpCount-1 ?>_Estado_de_CampaF1as"><?php echo $Page->PageBreakContent ?></span>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="programa"><div class="Estado_de_CampaF1as_programa"><span class="ewTableHeaderCaption"><?php echo $Page->programa->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="programa">
<?php if ($Page->SortUrl($Page->programa) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_programa">
			<span class="ewTableHeaderCaption"><?php echo $Page->programa->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" onclick="ewr_ShowPopup.call(this, event, 'Estado_de_CampaF1as_programa', false, '<?php echo $Page->programa->RangeFrom; ?>', '<?php echo $Page->programa->RangeTo; ?>');" id="x_programa<?php echo $Page->Cnt[0][0]; ?>"><span class="glyphicon glyphicon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_programa" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->programa) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->programa->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="campania"><div class="Estado_de_CampaF1as_campania"><span class="ewTableHeaderCaption"><?php echo $Page->campania->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="campania">
<?php if ($Page->SortUrl($Page->campania) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_campania">
			<span class="ewTableHeaderCaption"><?php echo $Page->campania->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" onclick="ewr_ShowPopup.call(this, event, 'Estado_de_CampaF1as_campania', false, '<?php echo $Page->campania->RangeFrom; ?>', '<?php echo $Page->campania->RangeTo; ?>');" id="x_campania<?php echo $Page->Cnt[0][0]; ?>"><span class="glyphicon glyphicon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_campania" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->campania) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->campania->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="asesor"><div class="Estado_de_CampaF1as_asesor"><span class="ewTableHeaderCaption"><?php echo $Page->asesor->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="asesor">
<?php if ($Page->SortUrl($Page->asesor) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_asesor">
			<span class="ewTableHeaderCaption"><?php echo $Page->asesor->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" onclick="ewr_ShowPopup.call(this, event, 'Estado_de_CampaF1as_asesor', false, '<?php echo $Page->asesor->RangeFrom; ?>', '<?php echo $Page->asesor->RangeTo; ?>');" id="x_asesor<?php echo $Page->Cnt[0][0]; ?>"><span class="glyphicon glyphicon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_asesor" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->asesor) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->asesor->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="fechatrans"><div class="Estado_de_CampaF1as_fechatrans"><span class="ewTableHeaderCaption"><?php echo $Page->fechatrans->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="fechatrans">
<?php if ($Page->SortUrl($Page->fechatrans) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_fechatrans">
			<span class="ewTableHeaderCaption"><?php echo $Page->fechatrans->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" onclick="ewr_ShowPopup.call(this, event, 'Estado_de_CampaF1as_fechatrans', false, '<?php echo $Page->fechatrans->RangeFrom; ?>', '<?php echo $Page->fechatrans->RangeTo; ?>');" id="x_fechatrans<?php echo $Page->Cnt[0][0]; ?>"><span class="glyphicon glyphicon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_fechatrans" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->fechatrans) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->fechatrans->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="ATENDIDO"><div class="Estado_de_CampaF1as_ATENDIDO" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->ATENDIDO->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="ATENDIDO">
<?php if ($Page->SortUrl($Page->ATENDIDO) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_ATENDIDO" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->ATENDIDO->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_ATENDIDO" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->ATENDIDO) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->ATENDIDO->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="PENDIENTE"><div class="Estado_de_CampaF1as_PENDIENTE" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->PENDIENTE->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="PENDIENTE">
<?php if ($Page->SortUrl($Page->PENDIENTE) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_PENDIENTE" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->PENDIENTE->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_PENDIENTE" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->PENDIENTE) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->PENDIENTE->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="TOTAL"><div class="Estado_de_CampaF1as_TOTAL" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->TOTAL->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="TOTAL">
<?php if ($Page->SortUrl($Page->TOTAL) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_TOTAL" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->TOTAL->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_TOTAL" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->TOTAL) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->TOTAL->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="CALIFICADO"><div class="Estado_de_CampaF1as_CALIFICADO" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->CALIFICADO->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="CALIFICADO">
<?php if ($Page->SortUrl($Page->CALIFICADO) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_CALIFICADO" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->CALIFICADO->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_CALIFICADO" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->CALIFICADO) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->CALIFICADO->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="NOINTERESADO"><div class="Estado_de_CampaF1as_NOINTERESADO" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->NOINTERESADO->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="NOINTERESADO">
<?php if ($Page->SortUrl($Page->NOINTERESADO) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_NOINTERESADO" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->NOINTERESADO->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_NOINTERESADO" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->NOINTERESADO) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->NOINTERESADO->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="OTROPROGRAMA"><div class="Estado_de_CampaF1as_OTROPROGRAMA" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->OTROPROGRAMA->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="OTROPROGRAMA">
<?php if ($Page->SortUrl($Page->OTROPROGRAMA) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_OTROPROGRAMA" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->OTROPROGRAMA->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_OTROPROGRAMA" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->OTROPROGRAMA) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->OTROPROGRAMA->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="FALLIDA"><div class="Estado_de_CampaF1as_FALLIDA" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->FALLIDA->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="FALLIDA">
<?php if ($Page->SortUrl($Page->FALLIDA) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_FALLIDA" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->FALLIDA->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_FALLIDA" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->FALLIDA) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->FALLIDA->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="PROCENT"><div class="Estado_de_CampaF1as_PROCENT" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->PROCENT->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="PROCENT">
<?php if ($Page->SortUrl($Page->PROCENT) == "") { ?>
		<div class="ewTableHeaderBtn Estado_de_CampaF1as_PROCENT" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->PROCENT->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Estado_de_CampaF1as_PROCENT" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->PROCENT) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->PROCENT->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_Estado_de_CampaF1as_programa"<?php echo $Page->programa->ViewAttributes() ?>><?php echo $Page->programa->GroupViewValue ?></span></td>
<?php } ?>
<?php if ($Page->campania->Visible) { ?>
		<td data-field="campania"<?php echo $Page->campania->CellAttributes(); ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_Estado_de_CampaF1as_campania"<?php echo $Page->campania->ViewAttributes() ?>><?php echo $Page->campania->GroupViewValue ?></span></td>
<?php } ?>
<?php if ($Page->asesor->Visible) { ?>
		<td data-field="asesor"<?php echo $Page->asesor->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Estado_de_CampaF1as_asesor"<?php echo $Page->asesor->ViewAttributes() ?>><?php echo $Page->asesor->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->fechatrans->Visible) { ?>
		<td data-field="fechatrans"<?php echo $Page->fechatrans->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Estado_de_CampaF1as_fechatrans"<?php echo $Page->fechatrans->ViewAttributes() ?>><?php echo $Page->fechatrans->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->ATENDIDO->Visible) { ?>
		<td data-field="ATENDIDO"<?php echo $Page->ATENDIDO->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Estado_de_CampaF1as_ATENDIDO"<?php echo $Page->ATENDIDO->ViewAttributes() ?>><?php echo $Page->ATENDIDO->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->PENDIENTE->Visible) { ?>
		<td data-field="PENDIENTE"<?php echo $Page->PENDIENTE->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Estado_de_CampaF1as_PENDIENTE"<?php echo $Page->PENDIENTE->ViewAttributes() ?>><?php echo $Page->PENDIENTE->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->TOTAL->Visible) { ?>
		<td data-field="TOTAL"<?php echo $Page->TOTAL->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Estado_de_CampaF1as_TOTAL"<?php echo $Page->TOTAL->ViewAttributes() ?>><?php echo $Page->TOTAL->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->CALIFICADO->Visible) { ?>
		<td data-field="CALIFICADO"<?php echo $Page->CALIFICADO->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Estado_de_CampaF1as_CALIFICADO"<?php echo $Page->CALIFICADO->ViewAttributes() ?>>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Estado_de_CampaF1as_NOINTERESADO"<?php echo $Page->NOINTERESADO->ViewAttributes() ?>>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Estado_de_CampaF1as_OTROPROGRAMA"<?php echo $Page->OTROPROGRAMA->ViewAttributes() ?>>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Estado_de_CampaF1as_FALLIDA"<?php echo $Page->FALLIDA->ViewAttributes() ?>>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_Estado_de_CampaF1as_PROCENT"<?php echo $Page->PROCENT->ViewAttributes() ?>><?php echo $Page->PROCENT->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<span data-class="tpgs<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_Estado_de_CampaF1as_ATENDIDO"<?php echo $Page->ATENDIDO->ViewAttributes() ?>><?php echo $Page->ATENDIDO->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->PENDIENTE->Visible) { ?>
		<td data-field="PENDIENTE"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_Estado_de_CampaF1as_PENDIENTE"<?php echo $Page->PENDIENTE->ViewAttributes() ?>><?php echo $Page->PENDIENTE->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->TOTAL->Visible) { ?>
		<td data-field="TOTAL"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_Estado_de_CampaF1as_TOTAL"<?php echo $Page->TOTAL->ViewAttributes() ?>><?php echo $Page->TOTAL->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->CALIFICADO->Visible) { ?>
		<td data-field="CALIFICADO"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_Estado_de_CampaF1as_CALIFICADO"<?php echo $Page->CALIFICADO->ViewAttributes() ?>>
<?php if ($Page->CALIFICADO->HrefValue <> "" || @$Page->CALIFICADO->LinkAttrs["onclick"] <> "") { ?>
...
<span data-class="tpgs<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_Estado_de_CampaF1as_NOINTERESADO"<?php echo $Page->NOINTERESADO->ViewAttributes() ?>>
<?php if ($Page->NOINTERESADO->HrefValue <> "" || @$Page->NOINTERESADO->LinkAttrs["onclick"] <> "") { ?>
...
<span data-class="tpgs<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_Estado_de_CampaF1as_OTROPROGRAMA"<?php echo $Page->OTROPROGRAMA->ViewAttributes() ?>>
<?php if ($Page->OTROPROGRAMA->HrefValue <> "" || @$Page->OTROPROGRAMA->LinkAttrs["onclick"] <> "") { ?>
...
<?php if ($Page->FALLIDA->Visible) { ?>
		<td data-field="FALLIDA"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_Estado_de_CampaF1as_FALLIDA"<?php echo $Page->FALLIDA->ViewAttributes() ?>>
<?php if ($Page->FALLIDA->HrefValue <> "" || @$Page->FALLIDA->LinkAttrs["onclick"] <> "") { ?>
...
<?php if ($Page->ATENDIDO->Visible) { ?>
		<td data-field="ATENDIDO"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_Estado_de_CampaF1as_ATENDIDO"<?php echo $Page->ATENDIDO->ViewAttributes() ?>><?php echo $Page->ATENDIDO->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->PENDIENTE->Visible) { ?>
		<td data-field="PENDIENTE"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_Estado_de_CampaF1as_PENDIENTE"<?php echo $Page->PENDIENTE->ViewAttributes() ?>><?php echo $Page->PENDIENTE->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->TOTAL->Visible) { ?>
		<td data-field="TOTAL"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_Estado_de_CampaF1as_TOTAL"<?php echo $Page->TOTAL->ViewAttributes() ?>><?php echo $Page->TOTAL->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->CALIFICADO->Visible) { ?>
		<td data-field="CALIFICADO"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_Estado_de_CampaF1as_CALIFICADO"<?php echo $Page->CALIFICADO->ViewAttributes() ?>>
<?php if ($Page->CALIFICADO->HrefValue <> "" || @$Page->CALIFICADO->LinkAttrs["onclick"] <> "") { ?>
<?php if ($Page->CALIFICADO->SumViewValue <> "" && $Page->CALIFICADO->SumViewValue <> "&nbsp;") { ?>
<a<?php echo $Page->CALIFICADO->LinkAttributes() ?>><?php echo $Page->CALIFICADO->SumViewValue ?></a>
<?php } else { echo "&nbsp;"; } ?>
...
<?php if ($Page->NOINTERESADO->Visible) { ?>
		<td data-field="NOINTERESADO"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_Estado_de_CampaF1as_NOINTERESADO"<?php echo $Page->NOINTERESADO->ViewAttributes() ?>>
<?php if ($Page->NOINTERESADO->HrefValue <> "" || @$Page->NOINTERESADO->LinkAttrs["onclick"] <> "") { ?>
<?php if ($Page->NOINTERESADO->SumViewValue <> "" && $Page->NOINTERESADO->SumViewValue <> "&nbsp;") { ?>
<a<?php echo $Page->NOINTERESADO->LinkAttributes() ?>><?php echo $Page->NOINTERESADO->SumViewValue ?></a>
<?php } else { echo "&nbsp;"; } ?>
...
<?php if ($Page->OTROPROGRAMA->Visible) { ?>
		<td data-field="OTROPROGRAMA"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_Estado_de_CampaF1as_OTROPROGRAMA"<?php echo $Page->OTROPROGRAMA->ViewAttributes() ?>>
<?php if ($Page->OTROPROGRAMA->HrefValue <> "" || @$Page->OTROPROGRAMA->LinkAttrs["onclick"] <> "") { ?>
<?php if ($Page->OTROPROGRAMA->SumViewValue <> "" && $Page->OTROPROGRAMA->SumViewValue <> "&nbsp;") { ?>
<a<?php echo $Page->OTROPROGRAMA->LinkAttributes() ?>><?php echo $Page->OTROPROGRAMA->SumViewValue ?></a>
<?php } else { echo "&nbsp;"; } ?>
...
<?php if ($Page->FALLIDA->Visible) { ?>
		<td data-field="FALLIDA"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_Estado_de_CampaF1as_FALLIDA"<?php echo $Page->FALLIDA->ViewAttributes() ?>>
<?php if ($Page->FALLIDA->HrefValue <> "" || @$Page->FALLIDA->LinkAttrs["onclick"] <> "") { ?>
<?php if ($Page->FALLIDA->SumViewValue <> "" && $Page->FALLIDA->SumViewValue <> "&nbsp;") { ?>
<a<?php echo $Page->FALLIDA->LinkAttributes() ?>><?php echo $Page->FALLIDA->SumViewValue ?></a>
<?php } else { echo "&nbsp;"; } ?>
