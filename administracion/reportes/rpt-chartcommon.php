...
<!--##
	...
	var bChartHtml5 = goCht.ChartHtml5 && UseFusionCharts() && !IsFCFChart(iChartType);
	...
	if (IsShowChart(goCht)) {
		...
		if (bUseEmbeddedImagesForEmail)
			...
			sPageBreakCheck = "$" + gsPageObj + "->Export == \"print\"";;
...
// Set up chart SQL
<!--## if (IsCrosstabChart) { ##-->
$SqlSelect = str_replace("<DistinctColumnFields>", $Table->DistinctColumnFields, $Table->SqlSelect());
...
<!--## } ##-->
<!--## if (bCustomViewSrcTable) { ##-->
$sSqlChartBase = "(" . ewr_BuildReportSql($SqlSelect, $Table->SqlWhere(), $Table->SqlGroupBy(), $Table->SqlHaving(), (EWR_IS_MSSQL) ? $Table->SqlOrderBy() : "", $<!--##=gsPageObj##-->->Filter, "") . ") EW_TMP_TABLE";
<!--## } else { ##-->
$sSqlChartBase = $Table->SqlFrom();
<!--## } ##-->
...
<!--## } else { ##-->
$sSql = $Chart->SqlSelectSeries . $sSqlChartBase;
$sSql = ewr_BuildReportSql($sSql, $Table->SqlWhere(), $Chart->SqlGroupBySeries, "", $Chart->SqlOrderBySeries, $<!--##=gsPageObj##-->->Filter, "");
<!--## } ##-->
...
<!--## } else { ##-->
$sSql = $SqlChartSelect . $sSqlChartBase;
$sSql = ewr_BuildReportSql($sSql, $Table->SqlWhere(), $Chart->SqlGroupBy, "", $Chart->SqlOrderBy, $<!--##=gsPageObj##-->->Filter, "");
<!--## } ##-->
$Chart->ChartSql = $sSql;
<!--##
...
##-->
$Chart->ChartDrillDownUrl = str_replace("f<!--##=j##-->", TEAencrypt($<!--##=gsPageObj##-->->GetDrillDownSQL($<!--##=sSourceFldObj##-->, "<!--##=sTargetFldParm##-->", <!--##=rowtype##-->, <!--##=parm##-->)), $Chart->ChartDrillDownUrl);
<!--##
				}
			}
		}
...
<!--## if (IsDashBoard && HasSourceReport && (!UseFusionCharts() || IsFCFChart(iChartType))) { // Dash board ##-->
...
<select id="chartordertype" name="chartordertype" onchange="this.form.submit();">
...
<!--## } ##-->
<!--## } ##-->
<div id="div_<!--##=sChartDivName##-->"></div>
<!-- grid component -->
<div id="div_<!--##=sChartDivName##-->_grid"></div>
</div>
...
<!--##include rpt-chartcommon-ext.php/chart_show_ext##-->

	} elseif ($Chart->ShowTempImage) { // Show temp image

		$TmpChartImage = ewr_TmpChartImage("chart_<!--##=gsTblVar##-->_<!--##=gsChartVar##-->");
		if ($TmpChartImage <> "") {
?>
<table class="ewChart"<!--##=sPageBreakTag##-->><tr><td><img src="<?php echo $TmpChartImage ?>" alt="" style="border: 0;" /></td></tr></table>
<?php
		}
	}
