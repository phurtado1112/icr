...
<!--## } ##-->
	...
	<!--## } ##-->
		...
		$sSql = ewr_BuildReportSql($this->SqlSelectGroup(), $this->SqlWhere(), $this->SqlGroupBy(), "", $this->SqlOrderByGroup(), $this->Filter, $sGrpSort);
...
if (!$<!--##=gsPageObj##-->->DrillDownInPanel) {
	...
	$<!--##=gsPageObj##-->->OtherOptions->Render("body");
}
?>
</div>
...
<!-- Top container (end) -->
<!--##
	CurCellPadding = (ew_ChartCount(2) > 0) ? " class=\"ewPadding\"" : "";
##-->
...
<!--##
	CurCellPadding = (bShowReport) ? " class=\"ewPadding\"" : "";
##-->
...
<!--## if (bColSearch) { ##-->
<!--##=sExpStart##-->
				<span class="ewTableHeaderPopup" onclick="ewr_ShowPopup.call(this, event, '<!--##=sColSessionFldVar##-->', <!--##=sColUseRange##-->, '<?php echo $<!--##=sColFldObj##-->->RangeFrom ?>', '<?php echo $<!--##=sColFldObj##-->->RangeTo ?>');" name="<!--##=sColFldVar##-->" id="<!--##=sColFldVar##-->"><span class="glyphicon glyphicon-filter"></span></span>
<!--##=sExpEnd##-->
<!--## } ##-->
			</div>
		</td>
...
<!--## if (bGenFilter) { ##-->
			<span class="ewTableHeaderPopup" onclick="ewr_ShowPopup.call(this, event, '<!--##=sSessionFldVar##-->', <!--##=sUseRange##-->, '<?php echo $<!--##=sFldObj##-->->RangeFrom; ?>', '<?php echo $<!--##=sFldObj##-->->RangeTo; ?>');" id="<!--##=sFldVar##-->"><span class="glyphicon glyphicon-filter"></span></span>
<!--## } ##-->
		</div>
	</td>
<?php } ?>
...
<?php
	...
	// Build detail SQL
	$sWhere = ewr_DetailFilterSQL($<!--##=arFirstGrpFld['FldObj']##-->, $<!--##=gsPageObj##-->->SqlFirstGroupField(), $<!--##=arFirstGrpFld['FldObj']##-->->GroupValue());
	...
	$sSql = ewr_BuildReportSql(str_replace("<DistinctColumnFields>", $<!--##=gsPageObj##-->->DistinctColumnFields, $<!--##=gsPageObj##-->->SqlSelect()), $<!--##=gsPageObj##-->->SqlWhere(), $<!--##=gsPageObj##-->->SqlGroupBy(), "", $<!--##=gsPageObj##-->->SqlOrderBy(), $sWhere, $<!--##=gsPageObj##-->->Sort);
...
<?php } elseif (!$<!--##=gsPageObj##-->->ShowHeader) { // No header displayed ?>
<!--##include rpt-phpcommon-table.php/report-header##-->
<?php } ?>
...
<!--##
	CurCellPadding = (ew_ChartCount(3) > 0) ? " class=\"ewPadding\"" : "";
##-->
...
<!--##
	CurCellPadding = (ew_ChartCount(4) > 0) ? " class=\"ewPadding\"" : "";
##-->
<div id="ewBottom">
<!--##=sHtmlExpEnd##-->
...
<!--##session phpfunction##-->
	...
	function GetColumns() {
		...
		$this->GetColumnValues($this->Filter);
	...
	function AccumulateSummary() {
		...
		for ($ix = 1; $ix < $cntx; $ix++) {
			...
			for ($iy = 0; $iy < $cnty; $iy++) {
				$valwrk = $this->Val[$ix];
				$this->Cnt[$ix][$iy]++;
				$this->Smry[$ix][$iy] = ewr_SummaryValue($this->Smry[$ix][$iy], $valwrk, $this->SummaryType());
	<!--## if (sSmryType == "AVG") { ##-->
	...
	<!--## } ##-->
				$sSql = ewr_BuildReportSql($<!--##=sFldObj##-->->SqlSelect, $this->SqlWhere(), $this->SqlGroupBy(), $this->SqlHaving(), $<!--##=sFldObj##-->->SqlOrderBy, $sFilter, "");
	...
	<!--## if (bColFldDateSelect  && ew_IsNotEmpty(sColDateFldName)) { ##-->
		...
		// Get distinct year
		$rsyear = $conn->Execute($this->SqlCrosstabYear());
		if ($rsyear) {
			while (!$rsyear->EOF) {
	...
	<!--## } ##-->
		...
		if ($this->RowTotalType == EWR_ROWTOTAL_GRAND) { // Grand total
			// Aggregate SQL
			$sSql = ewr_BuildReportSql(str_replace("<DistinctColumnFields>", $this->DistinctColumnFields, $this->SqlSelectAgg()), $this->SqlWhere(), $this->SqlGroupByAgg(), "", "", $this->Filter, "");
	...
	<!--## if (TABLE.TblRowSum) { ##-->
				$rowsmry = ewr_SummaryValue($rowsmry, $thisval, $this->SummaryType());
		<!--## if (sSmryType == "AVG") { ##-->
				$rowcnt += $thiscnt;
		<!--## } ##-->
	<!--## } ##-->
	...
	##-->
					...
					$url = str_replace("f<!--##=i##-->", TEAencrypt($this->GetDrillDownSQL($<!--##=sSourceFldObj##-->, "<!--##=sTargetFldParm##-->", $this->RowTotalType, $parm)), $url);
	<!--##
					} else {
	##-->
					$url = str_replace("f<!--##=i##-->", TEAencrypt($this->GetDrillDownSQL($<!--##=sSourceFldObj##-->, "<!--##=sTargetFldParm##-->", $this->RowTotalType<!--##=sColumnParm##-->)), $url);
	<!--##
					}
				}
			}
	...
	<!--##
			...
			if (arSourceFlds.length == arTargetFlds.length) {
				for (var i = 0, cnt = arTargetFlds.length; i < cnt; i++) {
					...
					var sTargetFldParm = TARGETFLD.FldVar.substr(2);
	##-->
					$url = str_replace("f<!--##=i##-->", TEAencrypt($this->GetDrillDownSQL($<!--##=sSourceFldObj##-->, "<!--##=sTargetFldParm##-->", 0<!--##=sColumnParm##-->)), $url);
	<!--##
				}
			}
	##-->
