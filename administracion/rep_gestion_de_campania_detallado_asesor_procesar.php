...
// Pie de página
        ...
        $consulta01 = "Select distinct agente, idusuario from campanias_reporte_view where idprograma=" . $idprograma;
        $res01 = bd_ejecutar_sql($consulta01);
        while ($fila01 = bd_obtener_fila($res01)) {
            $pdf->Cell(0, 10, utf8_decode($fila01['agente']), 0, 1, 'C');

            $consulta00 = "SELECT campania, idcampania FROM campanias_reporte_view where idprograma=" . $idprograma." and idusuario=" . $fila01['idusuario'];
            $res00 = bd_ejecutar_sql($consulta00);
            $pdf->SetFont('Arial', 'bu', 12);

            while ($fila00 = bd_obtener_fila($res00)) {
                $pdf->Cell(85, 10, utf8_decode('Campaña:'), 0, 0, 'R');
                $pdf->Cell(60, 10, utf8_decode($fila00['campania']), 0, 0, 'L');

                $pdf->Ln(10);

                $consulta = "SELECT count(idcliente) FROM incaecrm.clientes_x_programa_view where "
                        . "idprograma=" . $idprograma . " and idcampania=" . $fila00['idcampania']. " "
                        . "and idusuario=".$fila01['idusuario'];
                $res = bd_ejecutar_sql($consulta);
                $pdf->SetFont('Arial', '', 12);
                while ($fila = bd_obtener_fila($res)) {
                    $pdf->Cell(85, 6, utf8_decode('Total Leads de Asesor en la Campaña:'), 1, 0, 'L', true);
                ...
                $pdf->SetFont('Arial', 'b', 12);
                $pdf->Cell(0, 8, utf8_decode('1.- Totales del Asesor en la Campaña:'), 0, 1, 'L');
                ...
                while ($fila1 = bd_obtener_fila($res1)) {
                    $pdf->Cell(65, 6, utf8_decode('No Total de Leads Atendidos'), 1, 0, 'L', true);
