...
<!--##
	...
	for (var i = 0; i < nAllChtCount; i++) {
		if (GetChtObj(arAllCharts[i])) {
			if (IsShowChart(goCht)) {
				if (IsFCFChart(goCht.ChartType) || !UseFusionCharts())
					iFcfChartCnt += 1;
				bShowChart = true;
	...
	// Use embedded images for email
	bUseEmbeddedImagesForEmail = (iFcfChartCnt == 0);
	...
	if (bPrinterFriendly || bExportEmail) {
		sJsExpStart = "<?php if ($" + gsPageObj + "->Export == \"\" || $" + gsPageObj + "->Export == \"print\" || $" + gsPageObj + "->Export == \"email\" && (@$giFcfChartCnt > 0 || @$gsEmailContentType == \"url\")) { ?>";
		sJsExpEnd = "<?php } ?>";
	...
	if (bUseEmbeddedImagesForEmail) {
		...
		sChartExportShow = "$" + gsPageObj + "->Export == \"\" || ($" + gsPageObj + "->Export == \"print\" && $" + gsPageObj + "->CustomExport == \"\") || $" + gsPageObj + "->Export == \"email\"";
	}
	sChartExpStart = "<?php if (" + sChartExportShow + ") { ?>";
	if (bUseEmbeddedImagesForEmail && (bPrinterFriendly || bExportPdf || bExportEmail || bExportExcel && UsePhpExcel())) {
...
<!--##=sSkipPdfExpStart##-->
<table class="ewGrid"<?php echo $<!--##=gsPageObj##-->->ReportTableStyle ?>><tr>
