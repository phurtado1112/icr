...
<?php include_once "gestif3n_de_campaf1a_por_programa_seminariossmryinfo.php" ?>
<?php
...
$GestiF3n_de_CampaF1a_por_Programa_Seminarios_summary = NULL; // Initialize page object first

class crGestiF3n_de_CampaF1a_por_Programa_Seminarios_summary extends crGestiF3n_de_CampaF1a_por_Programa_Seminarios {
	...
	// Page object name
	var $PageObjName = 'GestiF3n_de_CampaF1a_por_Programa_Seminarios_summary';
	...
	function __construct() {
		...
		// Table object (GestiF3n_de_CampaF1a_por_Programa_Seminarios)
		if (!isset($GLOBALS["GestiF3n_de_CampaF1a_por_Programa_Seminarios"])) {
			$GLOBALS["GestiF3n_de_CampaF1a_por_Programa_Seminarios"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["GestiF3n_de_CampaF1a_por_Programa_Seminarios"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Gestión de Campaña por Programa Seminarios', TRUE);
...
//		$item->Visible = FALSE;
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'GestiF3n_de_CampaF1a_por_Programa_Seminarios_programa') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'GestiF3n_de_CampaF1a_por_Programa_Seminarios_campania') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'GestiF3n_de_CampaF1a_por_Programa_Seminarios_asesor') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'GestiF3n_de_CampaF1a_por_Programa_Seminarios_fechatrans') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_GestiF3n_de_CampaF1a_por_Programa_Seminarios_programa"])) {
			$this->LoadSelectionFromSession('programa');
		} elseif (@$_SESSION["sel_GestiF3n_de_CampaF1a_por_Programa_Seminarios_programa"] == EWR_INIT_VALUE) { // Select all
			$this->programa->SelectionList = "";
		}

		// Get campania selected values
		if (is_array(@$_SESSION["sel_GestiF3n_de_CampaF1a_por_Programa_Seminarios_campania"])) {
			$this->LoadSelectionFromSession('campania');
		} elseif (@$_SESSION["sel_GestiF3n_de_CampaF1a_por_Programa_Seminarios_campania"] == EWR_INIT_VALUE) { // Select all
			$this->campania->SelectionList = "";
		}

		// Get asesor selected values
		if (is_array(@$_SESSION["sel_GestiF3n_de_CampaF1a_por_Programa_Seminarios_asesor"])) {
			$this->LoadSelectionFromSession('asesor');
		} elseif (@$_SESSION["sel_GestiF3n_de_CampaF1a_por_Programa_Seminarios_asesor"] == EWR_INIT_VALUE) { // Select all
			$this->asesor->SelectionList = "";
		}

		// Get fechatrans selected values
		if (is_array(@$_SESSION["sel_GestiF3n_de_CampaF1a_por_Programa_Seminarios_fechatrans"])) {
			$this->LoadSelectionFromSession('fechatrans');
		} elseif (@$_SESSION["sel_GestiF3n_de_CampaF1a_por_Programa_Seminarios_fechatrans"] == EWR_INIT_VALUE) { // Select all
			$this->fechatrans->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			$this->PENDIENTE->ViewValue = ewr_FormatPercent($this->PENDIENTE->ViewValue, 0, -2, -2, -2);
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_GestiF3n_de_CampaF1a_por_Programa_Seminarios_$parm"] = "";
		$_SESSION["rf_GestiF3n_de_CampaF1a_por_Programa_Seminarios_$parm"] = "";
		$_SESSION["rt_GestiF3n_de_CampaF1a_por_Programa_Seminarios_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_GestiF3n_de_CampaF1a_por_Programa_Seminarios_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_GestiF3n_de_CampaF1a_por_Programa_Seminarios_$parm"];
		$fld->RangeTo = @$_SESSION["rt_GestiF3n_de_CampaF1a_por_Programa_Seminarios_$parm"];
	}
...
// Create page object
if (!isset($GestiF3n_de_CampaF1a_por_Programa_Seminarios_summary)) $GestiF3n_de_CampaF1a_por_Programa_Seminarios_summary = new crGestiF3n_de_CampaF1a_por_Programa_Seminarios_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$GestiF3n_de_CampaF1a_por_Programa_Seminarios_summary;
...
// Create page object
var GestiF3n_de_CampaF1a_por_Programa_Seminarios_summary = new ewr_Page("GestiF3n_de_CampaF1a_por_Programa_Seminarios_summary");

// Page properties
GestiF3n_de_CampaF1a_por_Programa_Seminarios_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = GestiF3n_de_CampaF1a_por_Programa_Seminarios_summary.PageID;

// Extend page with Chart_Rendering function
GestiF3n_de_CampaF1a_por_Programa_Seminarios_summary.Chart_Rendering = 
...
GestiF3n_de_CampaF1a_por_Programa_Seminarios_summary.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
<?php include "gestif3n_de_campaf1a_por_programa_seminariossmrypager.php" ?>
</div>
<?php } ?>
</td></tr></table>
<span data-class="tpb<?php echo $Page->GrpCount-1 ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios"><?php echo $Page->PageBreakContent ?></span>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="programa"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_programa"><span class="ewTableHeaderCaption"><?php echo $Page->programa->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="programa">
<?php if ($Page->SortUrl($Page->programa) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_programa">
			<span class="ewTableHeaderCaption"><?php echo $Page->programa->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" onclick="ewr_ShowPopup.call(this, event, 'GestiF3n_de_CampaF1a_por_Programa_Seminarios_programa', false, '<?php echo $Page->programa->RangeFrom; ?>', '<?php echo $Page->programa->RangeTo; ?>');" id="x_programa<?php echo $Page->Cnt[0][0]; ?>"><span class="glyphicon glyphicon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_programa" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->programa) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->programa->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="campania"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_campania"><span class="ewTableHeaderCaption"><?php echo $Page->campania->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="campania">
<?php if ($Page->SortUrl($Page->campania) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_campania">
			<span class="ewTableHeaderCaption"><?php echo $Page->campania->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" onclick="ewr_ShowPopup.call(this, event, 'GestiF3n_de_CampaF1a_por_Programa_Seminarios_campania', false, '<?php echo $Page->campania->RangeFrom; ?>', '<?php echo $Page->campania->RangeTo; ?>');" id="x_campania<?php echo $Page->Cnt[0][0]; ?>"><span class="glyphicon glyphicon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_campania" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->campania) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->campania->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="asesor"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_asesor"><span class="ewTableHeaderCaption"><?php echo $Page->asesor->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="asesor">
<?php if ($Page->SortUrl($Page->asesor) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_asesor">
			<span class="ewTableHeaderCaption"><?php echo $Page->asesor->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" onclick="ewr_ShowPopup.call(this, event, 'GestiF3n_de_CampaF1a_por_Programa_Seminarios_asesor', false, '<?php echo $Page->asesor->RangeFrom; ?>', '<?php echo $Page->asesor->RangeTo; ?>');" id="x_asesor<?php echo $Page->Cnt[0][0]; ?>"><span class="glyphicon glyphicon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_asesor" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->asesor) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->asesor->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="fechatrans"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_fechatrans" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->fechatrans->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="fechatrans">
<?php if ($Page->SortUrl($Page->fechatrans) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_fechatrans" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->fechatrans->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" onclick="ewr_ShowPopup.call(this, event, 'GestiF3n_de_CampaF1a_por_Programa_Seminarios_fechatrans', false, '<?php echo $Page->fechatrans->RangeFrom; ?>', '<?php echo $Page->fechatrans->RangeTo; ?>');" id="x_fechatrans<?php echo $Page->Cnt[0][0]; ?>"><span class="glyphicon glyphicon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_fechatrans" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->fechatrans) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->fechatrans->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="TOTAL"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_TOTAL"><span class="ewTableHeaderCaption"><?php echo $Page->TOTAL->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="TOTAL">
<?php if ($Page->SortUrl($Page->TOTAL) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_TOTAL">
			<span class="ewTableHeaderCaption"><?php echo $Page->TOTAL->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_TOTAL" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->TOTAL) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->TOTAL->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="ATENDIDO"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_ATENDIDO" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->ATENDIDO->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="ATENDIDO">
<?php if ($Page->SortUrl($Page->ATENDIDO) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_ATENDIDO" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->ATENDIDO->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_ATENDIDO" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->ATENDIDO) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->ATENDIDO->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="PENDIENTE"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_PENDIENTE" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->PENDIENTE->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="PENDIENTE">
<?php if ($Page->SortUrl($Page->PENDIENTE) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_PENDIENTE" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->PENDIENTE->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_PENDIENTE" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->PENDIENTE) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->PENDIENTE->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="CALIFICADO"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_CALIFICADO" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->CALIFICADO->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="CALIFICADO">
<?php if ($Page->SortUrl($Page->CALIFICADO) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_CALIFICADO" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->CALIFICADO->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_CALIFICADO" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->CALIFICADO) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->CALIFICADO->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="NOINTERESADO"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_NOINTERESADO" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->NOINTERESADO->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="NOINTERESADO">
<?php if ($Page->SortUrl($Page->NOINTERESADO) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_NOINTERESADO" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->NOINTERESADO->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_NOINTERESADO" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->NOINTERESADO) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->NOINTERESADO->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="OTROPROGRAMA"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_OTROPROGRAMA" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->OTROPROGRAMA->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="OTROPROGRAMA">
<?php if ($Page->SortUrl($Page->OTROPROGRAMA) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_OTROPROGRAMA" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->OTROPROGRAMA->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_OTROPROGRAMA" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->OTROPROGRAMA) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->OTROPROGRAMA->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="FALLIDA"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_FALLIDA" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->FALLIDA->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="FALLIDA">
<?php if ($Page->SortUrl($Page->FALLIDA) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_FALLIDA" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->FALLIDA->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_FALLIDA" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->FALLIDA) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->FALLIDA->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Otras"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_Otras" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->Otras->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Otras">
<?php if ($Page->SortUrl($Page->Otras) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_Otras" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->Otras->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_Otras" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Otras) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->Otras->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="PROCENT"><div class="GestiF3n_de_CampaF1a_por_Programa_Seminarios_PROCENT" style="text-align: center;"><span class="ewTableHeaderCaption"><?php echo $Page->PROCENT->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="PROCENT">
<?php if ($Page->SortUrl($Page->PROCENT) == "") { ?>
		<div class="ewTableHeaderBtn GestiF3n_de_CampaF1a_por_Programa_Seminarios_PROCENT" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->PROCENT->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer GestiF3n_de_CampaF1a_por_Programa_Seminarios_PROCENT" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->PROCENT) ?>',0);" style="text-align: center;">
			<span class="ewTableHeaderCaption"><?php echo $Page->PROCENT->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_programa"<?php echo $Page->programa->ViewAttributes() ?>><?php echo $Page->programa->GroupViewValue ?></span></td>
<?php } ?>
<?php if ($Page->campania->Visible) { ?>
		<td data-field="campania"<?php echo $Page->campania->CellAttributes(); ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_campania"<?php echo $Page->campania->ViewAttributes() ?>><?php echo $Page->campania->GroupViewValue ?></span></td>
<?php } ?>
<?php if ($Page->asesor->Visible) { ?>
		<td data-field="asesor"<?php echo $Page->asesor->CellAttributes(); ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_asesor"<?php echo $Page->asesor->ViewAttributes() ?>><?php echo $Page->asesor->GroupViewValue ?></span></td>
<?php } ?>
<?php if ($Page->fechatrans->Visible) { ?>
		<td data-field="fechatrans"<?php echo $Page->fechatrans->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->RecCount ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_fechatrans"<?php echo $Page->fechatrans->ViewAttributes() ?>><?php echo $Page->fechatrans->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->TOTAL->Visible) { ?>
		<td data-field="TOTAL"<?php echo $Page->TOTAL->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->RecCount ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_TOTAL"<?php echo $Page->TOTAL->ViewAttributes() ?>><?php echo $Page->TOTAL->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->ATENDIDO->Visible) { ?>
		<td data-field="ATENDIDO"<?php echo $Page->ATENDIDO->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->RecCount ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_ATENDIDO"<?php echo $Page->ATENDIDO->ViewAttributes() ?>><?php echo $Page->ATENDIDO->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->PENDIENTE->Visible) { ?>
		<td data-field="PENDIENTE"<?php echo $Page->PENDIENTE->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->RecCount ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_PENDIENTE"<?php echo $Page->PENDIENTE->ViewAttributes() ?>><?php echo $Page->PENDIENTE->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->CALIFICADO->Visible) { ?>
		<td data-field="CALIFICADO"<?php echo $Page->CALIFICADO->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->RecCount ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_CALIFICADO"<?php echo $Page->CALIFICADO->ViewAttributes() ?>><?php echo $Page->CALIFICADO->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->NOINTERESADO->Visible) { ?>
		<td data-field="NOINTERESADO"<?php echo $Page->NOINTERESADO->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->RecCount ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_NOINTERESADO"<?php echo $Page->NOINTERESADO->ViewAttributes() ?>><?php echo $Page->NOINTERESADO->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->OTROPROGRAMA->Visible) { ?>
		<td data-field="OTROPROGRAMA"<?php echo $Page->OTROPROGRAMA->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->RecCount ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_OTROPROGRAMA"<?php echo $Page->OTROPROGRAMA->ViewAttributes() ?>><?php echo $Page->OTROPROGRAMA->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->FALLIDA->Visible) { ?>
		<td data-field="FALLIDA"<?php echo $Page->FALLIDA->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->RecCount ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_FALLIDA"<?php echo $Page->FALLIDA->ViewAttributes() ?>><?php echo $Page->FALLIDA->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->Otras->Visible) { ?>
		<td data-field="Otras"<?php echo $Page->Otras->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->RecCount ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_Otras"<?php echo $Page->Otras->ViewAttributes() ?>><?php echo $Page->Otras->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->PROCENT->Visible) { ?>
		<td data-field="PROCENT"<?php echo $Page->PROCENT->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->RecCount ?>_GestiF3n_de_CampaF1a_por_Programa_Seminarios_PROCENT"<?php echo $Page->PROCENT->ViewAttributes() ?>><?php echo $Page->PROCENT->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php
		...
		if ($Page->ChkLvlBreak(3) && $Page->asesor->Visible) {
?>
<?php
			$Page->ResetAttrs();
...
<?php } ?>
<?php if ($Page->campania->Visible) { ?>
		<td data-field="campania"<?php echo $Page->campania->CellAttributes() ?>>&nbsp;</td>
<?php } ?>
<?php if ($Page->SubGrpFldCount + $Page->DtlFldCount - 1 > 0) { ?>
		<td colspan="<?php echo ($Page->SubGrpFldCount + $Page->DtlFldCount - 1) ?>"<?php echo $Page->asesor->CellAttributes() ?>><?php echo $ReportLanguage->Phrase("RptSumHead") ?> <?php echo $Page->asesor->FldCaption() ?>: <?php echo $Page->asesor->GroupViewValue ?> (<?php echo ewr_FormatNumber($Page->Cnt[3][0],0,-2,-2,-2) ?><?php echo $ReportLanguage->Phrase("RptDtlRec") ?>)</td>
<?php } ?>
</tr>
<?php
