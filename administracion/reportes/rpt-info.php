...
##-->
		$this-><!--##=gsFldParm##-->->SqlSelect = "SELECT DISTINCT <!--##=ew_Quote2(gsFld)##--><!--##=ew_Quote2(sGrpFld)##--> FROM " . $this->SqlFrom();
...
##-->
		...
		$this-><!--##=gsFldParm##-->->FldGroupSql = "<!--##=ew_DbGrpSql(goFld.FldGroupByType, goFld.FldGroupByInterval)##-->";
<!--##
			}
...
##-->
		...
		$this-><!--##=sColDateFldParm##-->->SqlSelect = "SELECT DISTINCT <!--##=ew_Quote2(sColDateFld)##--> FROM " . $this->SqlFrom();
...
##-->
	...
	function SqlFrom() { // From
		return "<!--##=ew_Quote2(sFrom)##-->";
	}
	function SqlSelect() { // Select
		return "SELECT <!--##=ew_Quote2(sSelect)##--> FROM " . $this->SqlFrom();
	}
<!--##
	sWhere = sWhere.trim();
...
<!--## } ##-->
	...
	function SqlGroupBy() { // Group By
		return "<!--##=ew_Quote2(sGroupBy)##-->";
	}
	function SqlHaving() { // Having
		return "";
	}
	function SqlOrderBy() { // Order By
		return "<!--##=ew_Quote2(sOrderBy)##-->";
	}

<!--## if (ew_IsNotEmpty(sDistinctSelect)) { ##-->
	function SqlDistinctSelect() {
		return "SELECT DISTINCT <!--##=ew_Quote2(sDistinctSelect)##--> FROM <!--##=ew_Quote2(sFrom)##-->";
	}
<!--##
...
<!--## } ##-->
	...
	function SqlDistinctOrderBy() {
		return "<!--##=ew_Quote2(sDistinctOrderBy)##-->";
	}
<!--## } ##-->
...
##-->
	function SqlCrosstabYear() {
		return "<!--##=ew_Quote2(sYearSql)##-->";
	}
<!--##
	}
##-->
	...
	// Get column values
	function GetColumnValues($filter = "") {
	...
	<!--##
		if (sColFldDateType == "q" || sColFldDateType == "m") {
	##-->
		$arColumnCaptions = explode(",", $this->ColumnCaptions());
		$arColumnNames = explode(",", $this->ColumnNames());
		$arColumnValues = explode(",", $this->ColumnValues());
	...
	##-->
		// Build SQL
		$sSql = ewr_BuildReportSql($this->SqlDistinctSelect(), $this->SqlDistinctWhere(), "", "", $this->SqlDistinctOrderBy(), $filter, "");
...
*/
	...
	<!--## if (sColFldDateType == "q" || sColFldDateType == "m") { ##-->
		...
		for ($i = 0; $i < $this->ColCount; $i++) {
			$sFld = ewr_CrossTabField($this->SummaryType(), $this->SummaryField(),
				$this->ColumnField(), $this->ColumnDateType(), $arColumnValues[$i], "", $arColumnNames[$i]);
			if ($sSqlFlds <> "")
				$sSqlFlds .= ", ";
			$sSqlFlds .= $sFld;
		}
	...
	<!--## } else { ##-->
		...
		$sSqlFlds = "";
		for ($colcnt = 1; $colcnt <= $this->ColCount; $colcnt++) {
			$sFld = ewr_CrossTabField($this->SummaryType(), $this->SummaryField(), $this->ColumnField(), $this->ColumnDateType(), $this->Col[$colcnt]->Value, "<!--##=sColFldQc##-->", "C" . $colcnt);
			if ($sSqlFlds <> "")
				$sSqlFlds .= ", ";
			$sSqlFlds .= $sFld;
		}
	...
	<!--## if (sColFldDateType == "q" || sColFldDateType == "m") { ##-->
				...
				$sChtFld = ewr_CrossTabField("SUM", $this->SummaryField(), $this->ColumnField(), $this->ColumnDateType(), $arColumnValues[$i], "");

	<!--## } else { ##-->

				$sChtFld = ewr_CrossTabField("SUM", $this->SummaryField(), $this->ColumnField(), $this->ColumnDateType(), $this->Col[$i+1]->Value, "<!--##=sColFldQc##-->");
...
<!--## if (TABLE.TblReportType == "crosstab" || TABLE.TblReportType == "summary") { ##-->
	...
	function SqlFirstGroupField() {
		return "<!--##=ew_Quote2(sFirstGroupFldSql)##-->";
	}
	function SqlSelectGroup() {
		return "SELECT DISTINCT " . $this->SqlFirstGroupField() . " FROM " . $this->SqlFrom();
	}
	function SqlOrderByGroup() {
		return "<!--##=ew_Quote2((sFirstGroupFldSql + " " + sFirstGroupFldOrderType).trim())##-->";
	}
<!--## } ##-->

<!--## if (TABLE.TblReportType == "crosstab") { ##-->
	function SqlSelectAgg() {
		return "SELECT <!--##=ew_Quote2(sSelectAgg)##--> FROM " . $this->SqlFrom();
	}
	function SqlGroupByAgg() {
		return "<!--##=ew_Quote2(sGroupByAgg)##-->";
	}
<!--## } else if (TABLE.TblReportType == "summary" || TABLE.TblReportType == "rpt") { ##-->
	function SqlSelectAgg() {
		return "SELECT <!--##=ew_Quote2(sAggFlds)##--> FROM " . $this->SqlFrom();
	}
	function SqlAggPfx() {
		return "<!--##=ew_Quote2(sAggPfx)##-->";
	}
	function SqlAggSfx() {
		return "<!--##=ew_Quote2(sAggSfx)##-->";
	}
