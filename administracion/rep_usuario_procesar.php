...
switch (filter_input(INPUT_POST, 'Submit')) {
    case "Cancelar":
        ...
        header("Location: rep_usuario_form.php");
        exit(0);
    ...
    case "Presentar":
        ...
        $pdf->SetWidths(array(10, 85, 25));
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->SetFillColor(0, 128, 255);
        $pdf->SetTextColor(255);

        for ($i = 0; $i < 1; $i++) {
            $pdf->Row(array('ID', utf8_decode('Agente'), 'Usuario'));
        }

        //Consulta a DB
        $consulta0 = "SELECT * FROM usuarios_reporte_view order by idusuario desc";
        ...
        for ($i = 0; $i < $numfilas; $i++) {
            ...
            if ($i % 2 == 1) {
                ...
                $pdf->SetTextColor(0);
                $pdf->Row(array($fila['idusuario'], utf8_decode($fila['nombre']), $fila['usuario']));
            } else {
