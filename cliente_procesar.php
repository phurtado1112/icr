...
if (isset($idcliente)) {
    ...
    if ($cantidad_cliente <= 1) {
        ...
        $consulta_idtransaccion="select max(idtrasaccion) as idtransaccion from transaccion where idcliente = '".$idcliente."'";
        $lista_idtransaccion=  bd_ejecutar_sql($consulta_idtransaccion);
        $fila_idtransaccion = bd_obtener_fila($lista_idtransaccion);
        $idtransaccion = $fila_idtransaccion['idtransaccion'];
        
        $consulta_actualiza_cliente_transaccion="update cliente_transaccion set idtransaccion = '".$idtransaccion."' where idcliente = '".$idcliente."'";
